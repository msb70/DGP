...
function renderSeguimiento() {
  const rutas = S.rutas.filter(r => ['liberada', 'en_ruta', 'cerrada', 'conciliada'].includes(r.estado)); const ent = S.paradas.filter(p => p.tipo === 'entrega' && rutas.some(r => r.id === p.ruta_id));
  $('seg-kpis').innerHTML = [['Rutas liberadas', rutas.length, `${rutas.filter(r => r.estado === 'en_ruta').length} en ruta`], ['Entregas atendidas', `${ent.filter(p => ['atendida', 'parcial', 'no_entregada'].includes(p.estado)).length}/${ent.length}`, `${ent.filter(p => p.estado === 'parcial').length} parciales · ${ent.filter(p => p.estado === 'no_entregada').length} no entregadas`], ['Incidencias', S.incidencias.length, S.incidencias.filter(i => i.tipo === 'demora').length + ' demoras'], ['Eventos recibidos', S.eventos.length, S.eventos.filter(e => e.offline).length + ' sincronizados desde cola offline']].map(k => `<div class="tile"><div class="l">${k[0]}</div><div class="v">${k[1]}</div><div class="s">${k[2]}</div></div>`).join('');
  $('seg-rutas').innerHTML = rutas.length ? rutas.map(r => { const v = veh(r.vehiculo_id) || {}; const st = S.paradas.filter(p => p.ruta_id === r.id).sort((a, b) => a.secuencia - b.secuencia); return `<div class="card flat veh" style="--vc:${v.color};padding:12px 14px"><div class="hd" style="margin-bottom:6px"><div><b>${r.codigo}</b> · ${v.placa} · ${r.conductor}</div>${rpill(r.estado)}${r.version > 1 ? ` <span class="pill p-info nodot">v${r.version}</span>` : ''}</div><div class="tw"><table><tbody>${st.map(s => { const c = s.cliente_id ? cli(s.cliente_id) : null; return `<tr><td class="num" style="width:30px">${s.secuencia}</td><td>${c ? esc(c.nombre) : esc(s.notas)}</td><td class="code">${s.eta}</td><td>${stpill(s.estado)}${s.resultado ? ` <span class="mini">${esc(s.resultado)}</span>` : ''}</td></tr>`; }).join('')}</tbody></table></div></div>`; }).join('') : '<div class="empty">Ninguna ruta liberada todavía. Verifica el cargue y libera la ruta desde Manifiesto.</div>';
  $('seg-log').innerHTML = S.eventos.slice(0, 80).map(e => { const r = S.rutas.find(x => x.id === e.ruta_id) || {}; const d = e.detalle || {}; return `<div><span>${new Date(e.created_at).toLocaleTimeString('es-PA', { hour: '2-digit', minute: '2-digit' })} ${r.codigo || ''}</span> <b>${e.tipo}</b> ${esc(d.texto || '')}${e.lat ? ` <span>· GPS ${(+e.lat).toFixed(4)},${(+e.lng).toFixed(4)}</span>` : ''}${e.offline ? ' <span>· offline→sync</span>' : ''} <span>· ${esc(e.actor || '')}</span></div>`; }).join('') || '<div><span>Sin eventos todavía.</span></div>';
  $('seg-note').textContent = DB.getMode() === 'supabase' ? 'Sondeo cada 10 s' : 'Modo local: mismo navegador';
}
